...
bool Player::movePiece(int file, int rank) {
    if(this->selected_piece != nullptr) {
        int capture_file = -1, capture_rank = -1;
        // piece_to_capture = malloc(sizeof(Piece));
        ...
        if(selected_piece->checkMove(file, rank, capture_file, capture_rank)) {
